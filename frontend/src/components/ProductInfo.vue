						...
						<table class="table table-striped">
							...
							<tbody>
								<tr v-for="x in product" :key="x.review_id">
									<td>{{ typeof x.review_rating === "number" ? x.review_rating.toFixed(1) : "N/A" }} stars</td>
									<td>{{ x.review_desc }}</td>
								</tr>
							</tbody>
						</table>
...
export default {
	...
	created() {
		axios
			.get(`http://54.236.93.253:3000/api/product-reviews/${this.$route.params.product_id}`)
			.then((res) => {
	...
	methods: {
		...
		handleSubmitForm() {
			let apiURL = `http://54.236.93.253:3000/api/checkout/`;
			axios
