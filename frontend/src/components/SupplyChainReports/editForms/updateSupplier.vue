...
export default {
	...
	created() {
		let apiURL = `http://54.236.93.253:3000/api/states`;
		axios
			.get(apiURL)
			.then((res) => {
				this.rows = res.data;
	...
	methods: {
		async getOne() {
			try {
				const res = await axios.get(`http://54.236.93.253:3000/api/suppliers/${this.$route.params.id}`);
		...
		handleUpdate() {
			let apiURL = `http://54.236.93.253:3000/api/updateSupplier/${this.$route.params.id}`;
			this.errors = [];
			axios
				.put(apiURL, this.supplier)
				.then(() => {
