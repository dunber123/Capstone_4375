<template>
	<div>
				...
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr v-for="x in rows" :key="x.supplier_id">
							...
							<td>
								<router-link :to="{ name: 'supplierUpdate', params: { id: x.supplier_id } }" class="btn btn-info is-small">Edit</router-link>
								<a class="btn btn-danger is-small" @click.prevent="deleteSupplier(x.supplier_id)">Delete</a>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div class="d-grid gap-4 d-md-flex justify-content-md-end">
			<a class="btn btn-primary me-md-2" href="/supplierAdd" role="button">Add New Supplier </a>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		deleteSupplier(id) {
			let apiURL = `http://54.236.93.253:3000/api/deleteSupplier/${id}`;
			console.log(id);
