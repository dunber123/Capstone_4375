...
productsRouter.get("/products", async (req, res) => {
	try {
		...
		res.status(500).send(`error [/api/product] : ${e}`);
	}
});
...
productsRouter.get("/products/:product_id", async (req, res) => {
	try {
		...
		const prod_id = await db.product.findUnique({
...
productsRouter.post("/addProduct", async (req, res) => {
	try {
		...
		res.status(500).send(`error [/api/addProduct] : ${e}`);
	}
});

//* New Product Review
//* host.com/api/newReview
productsRouter.post("/newReview", async (req, res) => {
	try {
